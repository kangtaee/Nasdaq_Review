...
본 프로젝트에 사용된 원시 뉴스 데이터는 NASDAQ 상장 종목 관련 기사로부터 수집된 RSS 정보를 기반으로 하며, 주요 컬럼은 다음과 같습니다:
...
> 이 중 감성 분석을 위해 `rgs_dt`, `tck_iem_cd`, `news_smy_ifo` 컬럼을 활용하였으며, 이후 감성 라벨링을 수행하였습니다.
...
이번 프로젝트에서 가장 많은 시간을 들인 부분 중 하나는 **뉴스 기사 약 4,000건에 대해 직접 감성 라벨을 부여한 작업**입니다.

- 키워드만으로 감정을 판단하는 방식의 한계를 느껴, 기사 내용을 **실제 투자자 입장에서 해석**하려고 노력했습니다.
- 예를 들어 `'Apple shares dip despite record revenue'`처럼 키워드는 긍정이지만, 문맥상으로는 주가 하락과 연결되기 때문에 중립이나 악재로 판단했습니다.

감성을 판단할 때는 다음과 같은 기준을 세우고 최대한 일관성 있게 적용했습니다:
...
- 해석이 모호한 문장은 따로 표시해두고, 이후 **반복 검토하거나 조원과 논의해 결정**했습니다.

> 총 4,000건 이상의 기사 요약문을 직접 읽고 분류하면서, 단순히 정확도를 높이기보다는  
> **실제로 투자에 참고할 수 있을 만한 감성 분류 기준을 만드는 데 중점을 뒀습니다.**
...
감성 라벨링이 완료된 뉴스 데이터는 다음과 같은 형태로 정리됩니다:
...
해당 뉴스가 발표된 시점과 그 이후의 주가 반응을 비교하기 위해, 아래와 같은 주가 데이터를 함께 사용하였습니다:
